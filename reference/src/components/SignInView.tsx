...
import { Key, ArrowRight, Eye, EyeOff, ShieldCheck, Activity, KeySquare, HelpCircle } from 'lucide-react';

interface SignInProps {
  onLoginSuccess: (email: string) => void;
}

export default function SignInView({ onLoginSuccess }: SignInProps) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center font-sans text-on-surface p-6 relative overflow-hidden bg-slate-100">
      {/* Decorative Orbs */}
      <div className="absolute w-[600px] h-[600px] bg-sky-200 rounded-full blur-[100px] opacity-40 -top-40 -left-40 animate-pulse pointer-events-none"></div>
      <div className="absolute w-[500px] h-[500px] bg-teal-200 rounded-full blur-[100px] opacity-40 -bottom-40 -right-40 animate-pulse pointer-events-none" style={{ animationDelay: '-5s' }}></div>

      <main className="w-full max-w-[1200px] grid md:grid-cols-12 items-center gap-12 z-10 relative">
        {/* Left Side: Branding / Corporate pitch */}
        <div className="hidden md:flex md:col-span-6 flex-col space-y-6">
          ...
          <div className="space-y-6">
            ...
            <p className="font-sans text-base text-on-surface-variant max-w-md leading-relaxed">
              Harness real-time data to optimize building efficiency, enhance tenant experience, and reduce operational costs with FacilityIQ.
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              <div className="flex items-center space-x-3 text-primary">
                ...
                <span className="text-xs font-semibold text-slate-700">Live Analytics</span>
              </div>
              <div className="flex items-center space-x-3 text-primary">
                <div className="p-2 bg-teal-50 text-teal-600 rounded-xl">
              ...
              <div className="flex items-center space-x-3 text-primary">
                ...
                <span className="text-xs font-semibold text-slate-700">Cloud Integrated</span>
              </div>
              <div className="flex items-center space-x-3 text-primary">
                <div className="p-2 bg-teal-50 text-teal-600 rounded-xl">
                  <KeySquare className="w-5 h-5" />
                </div>
                <span className="text-xs font-semibold text-slate-700">ESG Tracking</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 md:col-span-6 flex justify-center md:justify-end">
          <div className="glass-card w-full max-w-[480px] rounded-2xl p-8 flex flex-col space-y-6 border border-white bg-white/90">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
              ...
              <button
                ...
                className={`brand-gradient-btn w-full py-3.5 rounded-full font-display font-bold text-white shadow-lg flex items-center justify-center space-x-2 hover:opacity-95 transition-all active:scale-[0.98] ${
                  isLoading || isSuccess ? 'opacity-80 cursor-wait' : ''
                }`}
                style={isSuccess ? { backgroundColor: '#2EA056', backgroundImage: 'none' } : {}}
              >
                {isLoading ? (
                  <>
